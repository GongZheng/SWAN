...
int wbswan128_test(int weak_or_strong, enum swan_cipher_config_t cfg)
{
    ...
    printf("*****************************************\n");
    begin = start_rdtsc();
    swan_whitebox_128_init(key, 1, weak_or_strong, &swc_enc, cfg);
    end = end_rdtsc();
    ans = (end - begin);
    ...
    for (i = 0; i < 1000; i++)
    {
        swan_whitebox_128_encrypt(in, out, &swc_enct);
    }
    end = end_rdtsc();
    ans = (end - begin);
    ...
    swan_whitebox_128_init(key, 0, weak_or_strong, &swc_dec, cfg);
    begin = start_rdtsc();
    for (i = 0; i < 1000; i++)
    {
        swan_whitebox_128_decrypt(out, in, &swc_dec);
    }
    end = end_rdtsc();
    ans = (end - begin);
