...
int main()
{
    ...
    uint8_t prekey128[2 * ROUNDS64_K128 + 1][4];
    GenerateKey(key, 32, prekey128, KEY128);
    begin = start_rdtsc();
    for(int i = 0;i<TEST;i++){
        SWAN64_K128_decrypt_rounds(out, prekey128, ROUNDS64_K128, in);
    }
    end = end_rdtsc();
    ans = (end - begin);
    printf("the plaintext\n");
    ...
    uint8_t prekey256[2 * ROUNDS64_K256 + 1][4];
    GenerateKey(key, 64, prekey256, KEY256);
    begin = start_rdtsc();
    for (int i = 0; i < TEST; i++)
    {
        SWAN64_K256_decrypt_rounds(out, prekey256, ROUNDS64_K256, in);
    }
    end = end_rdtsc();
    ans = (end - begin);
    printf("the plaintext\n");
